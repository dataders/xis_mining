...
# # Subject Stats -----------------------------------------------------------
# 
#get grade mean&sd for t1, t2, t3,
#t1 to t2, t2 to t3, and t1 to t3 growth
...
by_grade <- year.report %>%
        ...
        summarize(t1.m = av(CriMean.t1), t1.s = s(CriMean.t1),
                  ...
                  t23.m = av(t23.growth), t23.s = s(t23.growth),
                  t13.m = av(t13.growth), t13.s = s(t13.growth)) %>%
        mutate_each(funs(round(.,2)), -Grade.Level) %>%
        select(-ends_with(".s"), -t13.m, -t23.m, -t3.m)

by_grade.ex <- year.report %>%
        filter(Grade.Level == "Grade 10", is.na(CriMean.t1) == TRUE) %>%
        select(-ends_with(".t3"), -ends_with(".s"), -starts_with("Student.Comment"), -starts_with("Sum"))