...
year.report <- GetYearReport() %>%
        ...
        mutate(Student.ID = "1000****",
               t12.growth.centered = round(t12.growth - av(t12.growth), 22)) %>%
        ungroup() %>%
        select( -Class.ID) %>%
        sample_n(100)
...
t1.top1000 <- t1.corpus %>% 
        GetDocumentTermMatrix(4,8, FALSE) %>%
        CollapseAndSortDTM() %>%
...
kable(t1.pruned , col.names = c("Phrases", "FrequencyXLength"),
...
Now we are getting somewhere! These are the phrases that were most used

What I want to do now is look at the language used to describe
